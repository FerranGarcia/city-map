...
The most simple thing we can do regarding to the points of interest is to show them according to the type belong to as we could see in figure ~\ref{fig:menu_poi}.
In addition, the points of interest basically has three different functions that are: Searching for specific point, modify an already existing point of interest and also create a new one pressing directly in the position desired in the map. 
...
\begin{figure}[h]
\centering
\includegraphics[width=0.4\textwidth]{map_plot}
\caption{Map}
\label{fig:map_plot}
\end{figure}
